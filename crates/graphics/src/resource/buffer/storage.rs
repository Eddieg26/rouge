...
pub struct StorageBufferDesc<T: ShaderType + WriteInto> {
    pub value: T,
...
impl<T: ShaderType + WriteInto> StorageBuffer<T> {
    pub fn new(device: &RenderDevice, desc: StorageBufferDesc<T>) -> Self {
...
impl<T: ShaderType + WriteInto> StorageBufferArray<T> {
    ...
    pub fn new_with_alignment(alignment: u64) -> Self {
        Self {
            data: encase::DynamicStorageBuffer::new_with_alignment(Vec::new(), alignment),
            buffer: None,
            label: None,
    ...
    pub fn with_usage(mut self, usage: wgpu::BufferUsages) -> Self {
        self.usage = usage | wgpu::BufferUsages::STORAGE;
        self
    }
