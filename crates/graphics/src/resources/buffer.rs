...
impl VertexBuffer {
    ...
    pub fn update<V: Vertex>(&mut self, device: &RenderDevice, offset: u64, vertices: &[V]) {
        if self.flags.is_write() {
            ...
            device
                .queue
                .write_buffer(&self.buffer, offset, bytemuck::cast_slice(vertices));
        }
    }
}
...
impl IndexBuffer {
    ...
    pub fn update(&mut self, device: &RenderDevice, offset: u64, indices: &Indices) {
        if self.flags.is_write() {
            ...
            device
                .queue
                .write_buffer(&self.buffer, offset, indices.bytes());
        }
    }
}
...
impl<T: BufferData> UniformBuffer<T> {
    ...
    pub fn update(&mut self, device: &RenderDevice, value: T) {
        self.value = value;
        let bytes = bytemuck::bytes_of(&self.value);
        device.queue.write_buffer(&self.buffer, 0, bytes);
    }
}
...
impl<T: BufferData> UniformBufferArray<T> {
    ...
    pub fn commit(&mut self, device: &RenderDevice) {
...
impl<T: BufferData> StorageBufferArray<T> {
    ...
    pub fn commit(&mut self, device: &RenderDevice) {
        ...
        if buffer_cap < size || ((self.dirty || self.buffer.is_none()) && size > 0) {
            ...
            device.queue.write_buffer(buffer, 0, &self.data);
        } else if size == 0 {
            self.buffer = None;
        }
    }
...
impl<T: BufferData> BatchedUniformBuffer<T> {
    ...
    pub fn commit(&mut self, device: &RenderDevice) {
        ...
        for (index, chunk) in self.data.chunks(self.batch_size).enumerate() {
            device.queue.write_buffer(&self.buffers[index], 0, chunk);
        }
